...
st.title("🔭 중력 렌즈 효과 시뮬레이터 (광원 & 관측자 고정)")
...
ax1.set_xlim(-50, 50)
ax1.set_ylim(-2, 2)
ax1.set_title("위치도: 렌즈, 행성, 광원 & 관측자 (고정)")
ax1.get_yaxis().set_visible(False)

# 광원 & 관측자 (같은 위치)
ax1.plot(source_x, -1, 'yellow', marker='*', markersize=18, label="광원")
ax1.plot(observer_x, -1.5, 'green', marker='^', markersize=12, label="관측자")
...
ax2.set_title("밝기 곡선 (렌즈가 지나가면서 측정)")
ax2.set_xlabel("렌즈의 위치")
